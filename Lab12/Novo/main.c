...
/** Retorna x * x, ou x^2 */
static double dquadrado(double x) {
    return x * x;
}


/** Remove tempos duplicados ou sobrepostos */
static void corrigir_instantes(Instante *instantes) {
...
static void calcular_distancia(DiferencaAsteroide *dif, double t) {
...
static int resolver_segundo_grau(
...
static inline void calcular_interseccao(
    ...
    * mesmo item */
    ...
    if(recebe_bit_(
        ...
        // printf(
        //     "Ignoramos intersecção já resolvida entre #%d e #%d\n",
        //     asteroide_0->indice,
        //     asteroide_1->indice
        // );
        return;
    ...
    /* Conferimos se ambos não pertencem a MST */
    if(recebe_bit(
        arestas_arv, asteroide_0->indice
    ) == 0 && recebe_bit(
        arestas_arv, asteroide_1->indice
    ) == 0) {
        // printf(
        //     "Ignoramos intersecção fora da árvore entre #%d e #%d\n",
        //     asteroide_0->indice,
        //     asteroide_1->indice
        // );
        return;
    ...
    // printf(
    //     "Fazemos intersecção entre #%d e #%d: ",
    //     asteroide_0->indice,
    //     asteroide_1->indice
    // );

    /* Calculamos a instersecção */
...
void quicksort_np(DiferencaAsteroide *vector, int start, int end) {
...
int main() {
    ...
    * asteróides */
    ...
    /* Inicializamos a matriz de intersecções já calculadas */
    inicializar_matriz_binaria(&interseccoes, combinacoes);
    ...
    while(instante_0 != NULL) {
        ...
        // printf(
        //     "\n\nt0 = %.7lf => t = %.7lf (aresta #%d: %d <=> %d e aresta"
        //     " #%d: %d <=> %d)\n",
        //     instante_0->t, t,
        ...
        for(int indice = 0; indice < combinacoes; indice++) {
            ...
            // printf(
            //     "inserida %d aresta #%d (%d <=> %d), distancia = %lf\n",
        ...
        * que contém os dois indices da função de distância */
        for(int indice = 0; indice < combinacoes; indice++) {
            ...
            // printf(
            //     "Calculamos intersecções para asteroide #%d (dist = %lf, "
            //     "t = %lf)\n",
            //     asteroide_0->indice, asteroide_0->distancia, t
            // );
            ...
            /* Pegamos todas as diferenças da mesma coluna que a coluna */
            for(int linha_ = 1; linha_ < num_asteroides; linha_++) {
                calcular_interseccao(
                    &matriz_dif,
                    asteroide_0,
            ...
            /* Pegamos todas as diferenças da mesma linha que a coluna */
            for(int coluna_ = 0; coluna_ < linha; coluna_++) {
                calcular_interseccao(
                    &matriz_dif,
                    asteroide_0,
        ...
        /* Verificamos se os instantes são válidos */
        corrigir_instantes(instantes);
    ...
    /* Liberamos a matriz de intersecções */
