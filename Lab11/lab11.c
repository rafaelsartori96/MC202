...
int inicializar_grafo(Grafo **grafo) {
    ...
    for(int i = 0; i < TAMANHO_MAXIMO; i++) {
        /* Verificamos se foi possível inicializar vetor de vizinhos */
        if(inicializar_vetor_circular_(
            &((*grafo)->vertices + i)->vizinhos
        ) != 1) {
            /* Liberamos os vetores já inicializados */
            for(int j = 0; j < i; j++) {
                liberar_vetor_circular_(&(((*grafo)->vertices + j)->vizinhos));
            }
            free((*grafo)->vertices);
            free(*grafo);
            (*grafo) = NULL;
...
void adiciona_vertice(Grafo *grafo, char *palavra_nova) {
    ...
    strcpy(vertice_novo->palavra, palavra_nova);

    /* Verificamos se a palavra muda em 1 caractere dos outros vértices, isto
    * é, procuramos vizinhos */
    for(
        int posicao_vizinho = 0;
        ...
        insere_valor_fim(&(vertice_novo->vizinhos), posicao_vizinho);
        insere_valor_fim(&(vertice_vizinho->vizinhos), posicao_nova);
    }
...
void liberar_grafo(Grafo **grafo) {
    ...
    {
        Vertice *vertice;
        for(int i = 0; i < (*grafo)->capacidade_vertices; i++) {
            vertice = ((*grafo)->vertices + i);

            /* Liberamos os vizinhos de cada vértice, mesmo não existente */
            liberar_vetor_circular_(&(vertice->vizinhos));
        }
        free((*grafo)->vertices);
    }
...
int busca_em_largura(Grafo *grafo, int vertice_inicial) {
    ...
    while(fila->tamanho > 0) {
        ...
        * distância entre dois vértices) */
        ...
        for(int i = 0; i < vertice->vizinhos.tamanho; i++) {
            int vizinho;
            recebe_valor(&(vertice->vizinhos), i, &vizinho);
...
int passo_de_edicao(char *palavra_1, char *palavra_2) {
    ...
    while(edicoes <= 1) {
        ...
        /* Verificamos se as letras são diferentes e se fizemos mais edições
        * do que devíamos */
