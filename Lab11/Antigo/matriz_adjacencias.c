...
int inicializar_matriz_adjacencias(
    ...
    (*matriz) = malloc(sizeof(MatrizAdjacencias));
    if((*matriz) == NULL) {
        return -1;
    }
...
int recebe_bit_(MatrizAdjacencias *matriz, int linha_, int coluna_) {
...
int define_bit_verdadeiro_(MatrizAdjacencias *matriz, int linha_, int coluna_) {
    /* Calculamos a posição do bit no vetor linearizado */
    int linha = max(linha_, coluna_);
    int coluna = min(linha_, coluna_);
...
int define_bit_falso_(MatrizAdjacencias *matriz, int linha_, int coluna_) {
    /* Calculamos a posição do bit no vetor linearizado */
    int linha = max(linha_, coluna_);
    int coluna = min(linha_, coluna_);
