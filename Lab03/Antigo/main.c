...
char caractere_porcentagem(float porcentagem) {
    if(0.75d <= porcentagem && porcentagem <= 1.0d) {
        return '#';
    } else if(0.25d <= porcentagem && porcentagem < 0.75d) {
        return '-';
    } else {
        return ' ';
    }
        ...
        for(i = 0; i < 8; i++) {
            ...
            */
            ...
            if(fim >= inicio) {
                ...
                blocos[i] += (double) ((fim - inicio) / (disco->capacidade / 8.0d));
            }
        }
    }
